...
ovrLexer::ovrLexer( const char * source, const size_t sourceLength, char const * punctuation )
	...
	, Punctuation( NULL )
{
	size_t len = punctuation == NULL ? 0 :OVR_strlen( punctuation );
	if ( len == 0 )
	{
	...
	else
	{
		...
		OVR_strcpy( Punctuation, len + 1, punctuation );
	}
}
...
ovrLexer::ovrLexer( const char * source )
	: ovrLexer( source, OVR_strlen( source ), NULL )
{
}
...
ovrLexer::ovrLexer( const char * source, char const * punctuation )
	: ovrLexer( source, OVR_strlen( source ), punctuation )
{
}

//==============================
// ovrLexer::ovrLexer
ovrLexer::ovrLexer( MemBufferT< uint8_t > const & source, char const * punctuation )
	: ovrLexer( (const char*)( static_cast< uint8_t const* >( source ) ), source.GetSize(), punctuation )
{
}
...
ovrLexer::ovrResult ovrLexer::SkipWhitespace( char const * & p, char const * source, size_t const sourceLength )
{
...
void ovrLexer::EmitCodePoint( uint32_t const ch, char * buffer, intptr_t & bufferOfs, size_t const bufferSize,
		char * token, size_t const maxTokenSize ) const
{
	...
	for ( ; ; )
	{
		...
		if ( inComment )
		{
			if ( ch == '*' && PeekNextChar() == '/' )
			{
				...
				if ( res != LEX_RESULT_OK ) { return res; }
			}
			continue;
		}
		else if ( inQuotes && ch == '\\' )
		...
		if ( static_cast< size_t >( bufferOfs + encodeSize ) >= BUFF_SIZE - 1 || static_cast< size_t >( bufferOfs + encodeSize + 1 ) >= maxTokenSize )
		{
			// truncation
			UTF8Util::EncodeChar( buffer, &bufferOfs, '\0' );
			CopyResult( buffer, token, maxTokenSize );
...
ovrLexer::ovrResult ovrLexer::ExpectToken( char const * expectedToken, char * token, size_t const maxTokenSize )
{
	ovrResult res = NextToken( token, maxTokenSize );
	...
	if ( OVR_strcmp( token, expectedToken ) != 0 )
	{
		return LEX_RESULT_UNEXPECTED_TOKEN;
	}
	return LEX_RESULT_OK;
...
ovrLexer::ovrResult ovrLexer::ExpectPunctuation( char const * punc, char * token, size_t const maxTokenSize )
{
	ovrResult res = NextToken( token, maxTokenSize );
	if ( res != LEX_RESULT_OK )
	{
	...
	size_t tokenLen = OVR_strlen( token );
	if ( tokenLen == 1 )
	{
		int len = static_cast< int >( OVR_strlen( punc ) );
		for ( int i = 0; i < len; ++i )
		{
			if ( punc[i] == token[0] )
			{

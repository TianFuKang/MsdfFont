...
#include <stdint.h>
#include <ft2build.h>
#include <Kernel/OVR_System.h> // for OVR::System::Init()
#include <OVR_JSON.h>
#include <sstream>
#include <gl/GL.h>
#include <intrin.h>
...
static bool writeBmpHeader(FILE *file, int width, int height, int &paddedWidth) {
	...
	const uint32_t bitmapSize = paddedWidth * height * 3;
...
{
	FILE *file = fopen("ktx.bmp", "wb");
	if (!file)
		return false;
	int paddedWidth;
	writeBmpHeader(file, srcWidth, srcHeight, paddedWidth);
	...
	header.glInternalFormat = GL_RGBA8;
	...
	size_t destRowSize = srcWidth * 4;
	uint8_t * destRowBuffer = new uint8_t[destRowSize];
	for ( int y = 0; y < srcHeight; ++y )
	{
		size_t srcRowIdx = ( y * srcRowSize );
		for ( int x = 0, t = 0; x < srcWidth; ++x)
		{
			uint8_t bgr[3] = {
				buffer[srcRowIdx + (x * 4) + 0],
				buffer[srcRowIdx + (x * 4) + 1],
				buffer[srcRowIdx + (x * 4) + 2]
			};
			fwrite(bgr, sizeof(uint8_t), 3, file);
			uint8_t temp0 = destRowBuffer[t++] = buffer[srcRowIdx + (x * 4) + 0];
			uint8_t temp1 = destRowBuffer[t++] = buffer[srcRowIdx + (x * 4) + 1];
			uint8_t temp2 = destRowBuffer[t++] = buffer[srcRowIdx + (x * 4) + 2];
			uint8_t temp3 = destRowBuffer[t++] = buffer[srcRowIdx + (x * 4) + 3];
		}
		fwrite(padding, 1, paddedWidth - 3 * srcWidth, file);
...
static int CreateBitmapFont( int const argc, char const * argv[], FT_Library & ftLibrary )
{
	...
	for (int i = 3; i < argc; ++i)
	{
		...
		else if (_stricmp(argv[i], "-ew") == 0)
		{
			ASSIGN_NEXT_ARGF(edgeWidth);
		}
		else if (_stricmp(argv[i], "-remap") == 0)
		{
		...
		else if (_stricmp(argv[i], "-fb") == 0 || _stricmp(argv[i], "-fallback") == 0)
		{
	...
	FT_Face ftFace;
	error = FT_New_Face( ftLibrary, inFileName, fontIdx, &ftFace );
	...
	*/
	...
	if (!ft) return -1;
	FontHandle *font = loadFont(ft, inFileName);
	if (!font) {
...

	{
		...
		OVR::String allCodepointsStr = OVR::String::Format( "Unicode code points for '%s':\nTotal code points: %i\n", outFileName, static_cast< int >( glyphToUnicodeMap.size() ) );
		for ( size_t i = 0; i < glyphToUnicodeMap.size(); ++i )
		{
			OVR::String temp;
			temp = OVR::String::Format( "%d: ", glyphToUnicodeMap[i] );
			temp.AppendChar( glyphToUnicodeMap[i] );
			temp.AppendChar( '\n' );
		...
		tmp[0] = outFileName[strlen(outFileName) - 3];
		tmp[1] = outFileName[strlen(outFileName) - 2];
		tmp[2] = outFileName[strlen(outFileName) - 1];
		tmp[3] = '\0';
		ext = tmp;
		int i = strlen(outFileName) - 5, j = 0;
		for (; outFileName[i] != '/' && outFileName[i] != '\\'; i--, j++) {
			tmp[j] = outFileName[i];
		}
		tmp[j] = '\0';
		filename = tmp;
		...
		/*PathUtils::AppendPath( path, filename );*/
		path += filename;
		path += "_allcodepoints.txt";
		...
		fwrite( allCodepointsStr.ToCStr(), allCodepointsStr.GetSize(), 1, outf );
		fclose( outf );
	}
	...
	for ( int32_t i = 0; i < static_cast< int32_t >(glyphToUnicodeMap.size() ); ++i )
	{
		...
		*/
		...
		//FreetypeHandle *ft = initializeFreetype();
		//if (!ft) return -1;
		//FontHandle *font = loadFont(ft, "NotoSansCJKjp-Regular.otf");
		//if (!font) {
		//	//deinitializeFreetype(ft);
		//	std::cout << "Failed to load font file." << std::endl;
		//	continue;
		//}
		if (!loadGlyph(shape, font, charCode, &glyphAdvance)) {
			std::cout << "Failed to load glyph from font file." << std::endl;
			continue;
		}
		//destroyFont(font);
		//deinitializeFreetype(ft);
		...
		if ( glyphIdx == 0 )
		{
			...
			{				
				...
				if ( glyphIdx > 0 )
				{
					std::cout << "No glyph for character code " << charCode << ", using fallback " << fallbackFontFiles[f].c_str() << "\n";
		...
		/*std::cout << "FT_LOAD_DEFAULT:" << FT_LOAD_DEFAULT << std::endl;
		std::cout << "FT_LOAD_NO_SCALE:" << FT_LOAD_NO_SCALE << std::endl;
		std::cout << "FT_LOAD_NO_HINTING:" << FT_LOAD_NO_HINTING << std::endl;
		std::cout << "FT_LOAD_RENDER:" << FT_LOAD_RENDER << std::endl;
		std::cout << "FT_LOAD_NO_BITMAP:" << FT_LOAD_NO_BITMAP << std::endl;
		std::cout << "FT_LOAD_VERTICAL_LAYOUT:" << FT_LOAD_VERTICAL_LAYOUT << std::endl;
		std::cout << "FT_LOAD_FORCE_AUTOHINT:" << FT_LOAD_FORCE_AUTOHINT << std::endl;
		std::cout << "FT_LOAD_CROP_BITMAP:" << FT_LOAD_CROP_BITMAP << std::endl;
		std::cout << "FT_LOAD_PEDANTIC:" << FT_LOAD_PEDANTIC << std::endl;
		std::cout << "FT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH:" << FT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH << std::endl;
		std::cout << "FT_LOAD_NO_RECURSE:" << FT_LOAD_NO_RECURSE << std::endl;
		std::cout << "FT_LOAD_IGNORE_TRANSFORM:" << FT_LOAD_IGNORE_TRANSFORM << std::endl;
		std::cout << "FT_LOAD_MONOCHROME:" << FT_LOAD_MONOCHROME << std::endl;
		std::cout << "FT_LOAD_LINEAR_DESIGN:" << FT_LOAD_LINEAR_DESIGN << std::endl;
		std::cout << "FT_LOAD_NO_AUTOHINT:" << FT_LOAD_NO_AUTOHINT << std::endl;
		std::cout << "FT_LOAD_SBITS_ONLY:" << FT_LOAD_SBITS_ONLY << std::endl;*/

		const int32_t loadFlags = FT_LOAD_DEFAULT/*FT_LOAD_RENDER*/;
		...
		{
			error = FT_Render_Glyph( face->glyph, /*FT_RENDER_MODE_NORMAL*/FT_RENDER_MODE_NORMAL);
			if ( error )
			{
		...
		/*if (face->glyph->bitmap.rows != 0) {
			for (int y = 0; y < face->glyph->bitmap.rows; ++y)
			{
				...
				{
					std::cout << int(face->glyph->bitmap.buffer[face->glyph->bitmap.width * y + x]) << " ";
				}
				std::cout << std::endl;
			}
			int temp = 0;
		...
		*/
		...
		if (!shape.validate())
			std::cout << "The geometry of the loaded shape is invalid." << std::endl;
		shape.normalize();
		if (yFlip)
		...
		if (autoFrame) {
			...
			if (frame.x <= 0 || frame.y <= 0)
				std::cout << "Cannot fit the specified pixel range." << std::endl;
		...
		simulate8bit(msdf);
		Bitmap<float> render(512, 512);
		renderSDF(render, msdf, avgScale*range);
		savePng(render, "output.png");
		for (int y = 0; y < msdf.height(); ++y)
			for (int x = 0; x < msdf.width(); ++x) {
				msdf(x, y).r = msdfgen::clamp(int(msdf(x, y).r * 0x100), 0xff);
				msdf(x, y).g = msdfgen::clamp(int(msdf(x, y).g * 0x100), 0xff);
				msdf(x, y).b = msdfgen::clamp(int(msdf(x, y).b * 0x100), 0xff);
			}
		//

		//std::cout << std::endl << ftBitmap.pixel_mode << std::endl;
		if ( ftBitmap.pixel_mode == FT_PIXEL_MODE_GRAY)////////////////////////////////////////////////////////////////////////////////
		{
			...
			for ( int y = 0; y < ftBitmap.rows; ++y )
			{
				...
				{
					uint8_t r_ = msdf(x, y).r;
					uint8_t g_ = msdf(x, y).g;
					uint8_t b_ = msdf(x, y).b;
					uint8_t alpha = alphaValue < 0 ? r_ : static_cast< uint8_t >(alphaValue);
					g.Bitmap->SetPixelRGBA(x + (hpad >> 1), y + (vpad >> 1), r_, g_, b_, alpha);
				}
			}
			if ( grid )
			{
...
*/
	...
	if (signedDistanceField == true) {
		imageFileName = sdfFileName;
	}
	else {
		imageFileName = tgaFileName;
	}
...
static void PrintUsage()
{
	...
	std::cout << "-co <f> : floating point number specifying and offset for the 'center' of the distance range.\n";
